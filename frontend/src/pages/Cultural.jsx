...
const Cultural = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        setLoading(true);
        const response = await eventService.getPublishedEvents("cultural");
        setEvents(response.data || []);
      } catch (err) {
        console.error("Error fetching cultural events:", err);
        setError(err.message || "Failed to load cultural events");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex w-full">
      ...
      <main className="flex-1 px-10 py-8">
        ...
        <section className="mb-12">
          <h2 className="text-3xl font-bold mb-2">Cultural Events 🎭</h2>
          <p className="text-slate-600 mb-8">
            Celebrate culture through our cultural events
          </p>

          {loading && <div className="text-center py-12">Loading cultural events...</div>}
          ...
          {!loading && events.length === 0 && (
            <div className="text-center py-12 text-slate-500">
              No cultural events available yet
            </div>
          )}
