...
const Mentorship = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        setLoading(true);
        const response = await eventService.getPublishedEvents("club");
        setEvents(response.data || []);
      } catch (err) {
        console.error("Error fetching mentorship programs:", err);
        setError(err.message || "Failed to load mentorship programs");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex w-full">
      ...
      <main className="flex-1 px-10 py-8">
        ...
        <section className="mb-12">
          <h2 className="text-3xl font-bold mb-2">Mentorship 👥</h2>
          <p className="text-slate-600 mb-8">
            Learn from experienced mentors and guides
          </p>

          {loading && <div className="text-center py-12">Loading mentorship programs...</div>}
